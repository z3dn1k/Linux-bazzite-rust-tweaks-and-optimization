...
launch option commands for higher fps and tut for how to actually play rust on linux (specificaly in my case bazzite)
...
```gamescope -w 1280 -h 960 -r 320 -S stretch -- %command% -screen-width 1280 -screen-height 960 -rate 320 -window-mode exclusive -nolog -high -force-feature-level-11-0 +gc.buffer 2048```
  ...
  -this line will set rust into 1280 by 960 resolution without black bars and set refresh rate to 320 (-w 1280 -h 960 and -rate 320)

### 2. What servers to play on

  -You cannot play on Official Facepunch servers or any community server that enforces strict EAC. If you try, you will likely be kicked after a minute or two with an "EAC Disconnected" error. So you have to play on servers specificaly configured for linux/No-EAC
